...
public:
  Program(int argc, char **argv)
      : argc(argc), argv(argv), bin_size(1),
        num_of_result_bins(NUM_OF_RESULT_BINS_DEFAULT), postfix(false),
        help_flag(false), output_flag(false), input_flag(false),
    ...
    while ((c = getopt(argc, argv, "i:o:b:r:ph")) != -1) {
      ...
      if (r_in_switch) {
        ...
        if (!(tmp >> num_of_result_bins)) {
          std::cout << "ERROR: UNABLE TO READ NUMBER OF BINS IN RESULT, "
                       "SETTING NUM_OF_RUSULT_BINS TO 100 (DEFAULT)."
                    << std::endl;
  ...
  bool read_input() {
    ...
    if (input_flag) { // We put the whole file into the input_buffer
      ...
      if (input_file.is_open()) {
        input_buffer
            << input_file.rdbuf(); // Load the file into the input_buffer
        input_file.close();
      } else {
        std::cerr << "ERROR: UNABLE TO OPEN THE INPUT FILE " << input_file_name
                  << std::endl;
        return false;
      }
  ...
  bool print_help() {
    ...
    if (help_flag) {
      ...
      std::cout << "    -b: bin_size - size of the bins in which the "
                ...
                << std::endl;
      std::cout << "    -r: how many bins to use during result presentation, "
                   "default = "
    ...
    if (postfix) {
      if (!expression.parse_postfix_input(input_buffer)) {
        ...
        std::cerr << "     - DIVISION BY ZERO (BEWARE OF DISTRIBUTIONS WHICH "
                     "INCLUDE ZERO)"
                  << std::endl;
        std::cerr << "     - WRONG INPUT (ERROR IN FORMAT - NOT ENOUGH "
                     "OPERANDS, TOO MANY OPERANDS, NO MATCHING PARENTHESES,.."
                  << std::endl;
        return false;
      }
    } else {
...
int main(int argc, char **argv) {
  ...
  if (!program.parse_arguments())
    return 1;
  if (program.print_help())
    return 0;
  if (!program.read_input())
    return 1;
  if (!program.compute())
    return 1;
  if (!program.output())
