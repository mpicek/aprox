...
template <typename real> struct Parsed_arguments {
  ...
  Parsed_arguments()
      : bin_size(1), num_of_result_bins(NUM_OF_RESULT_BINS_DEFAULT),
        postfix(false), help_flag(false), output_flag(false), input_flag(false),
...
Parsed_arguments<real> parse_arguments(int argc, char **argv) {
  ...
  while ((c = getopt(argc, argv, "i:o:b:r:ph")) != -1) {
    ...
    // In switch you are unable to create new variables TODO why?? ... this is
    // the solution
    if (s_in_switch) {
      std::stringstream tmp(bin_size_char);
      if (!(tmp >> args.bin_size)) {
        std::cout << "ERROR: UNABLE TO READ BIN SIZE, SETTING BIN_SIZE TO 1 "
                     "(DEFAULT)."
                  << std::endl;
        args.bin_size = 1;
      }
    ...
    if (r_in_switch) {
      ...
      if (!(tmp >> args.num_of_result_bins)) {
        std::cout << "ERROR: UNABLE TO READ NUMBER OF BINS IN RESULT, SETTING "
                     "NUM_OF_RUSULT_BINS TO 100 (DEFAULT)."
                  << std::endl;
 ...
 */
template <typename real> bool print_help(Parsed_arguments<real> &args) {
  if (args.help_flag) {
    std::cout << "Usage: aprox [-i in] [-o out] [-s interval size]"
              << std::endl;
    ...
    std::cout << "    -b: bin_size - size of the bins in which the "
                 "distributions are stored, default = 1"
              << std::endl;
    std::cout
        << "    -r: how many bins to use during result presentation, default = "
        << NUM_OF_RESULT_BINS_DEFAULT << std::endl;
 ...
 */
template <typename real>
bool compute(Parsed_arguments<real> &args, Expression<real> &expression,
             std::stringstream &input_buffer) {
  if (args.postfix) {
    if (!expression.parse_postfix_input(input_buffer)) {
      std::cerr << "ERROR: PROBLEM DURING EVALUATION OCCURED - PROBABLY WHAT "
                   "HAPPENED:"
                << std::endl;
      std::cerr << "     - DIVISION BY ZERO (BEWARE OF DISTRIBUTIONS WHICH "
                   "INCLUDE ZERO)"
                << std::endl;
      std::cerr << "     - WRONG INPUT (ERROR IN FORMAT - NOT ENOUGH OPERANDS, "
                   "TOO MANY OPERANDS, NO MATCHING PARENTHESES,.."
                << std::endl;
      return false;
    }
  } else {
...
int main(int argc, char **argv) {
  ...
  if (args.error_occurred)
    return 1;
  if (print_help<real>(args))
    return 0;
  if (!read_input<real>(args, input_buffer))
    return 1;
  if (!compute<real>(args, expression, input_buffer))
    return 1;
  if (!output<real>(args, expression))
    return 1;
